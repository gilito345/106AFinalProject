...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            for d in np.arange(r, -1, -min(self._x_res, self._y_res)):
                x = sensor_x + d * np.cos(angle)
                y = sensor_y + d * np.sin(angle)
                (grid_x, grid_y) = self.PointToVoxel(x, y)
                #print(d)

                # if (grid_x < self._x_min or grid_x > self._x_max or grid_y < self._y_min or grid_y > self._y_max):
                #     last_x = grid_x
                #     last_y = grid_y
                #     continue
                ...
                if (d == r):
                    ...
                    if (grid_x == last_x and grid_y == last_y):
                        continue
